...
function renderInfo(country) {
  ...
  clone.querySelector('.capital').textContent = `Столиця: ${country.capital ? country.capital[0] : '—'}`;
  clone.querySelector('.population').textContent = `Населення: ${country.population.toLocaleString()}`;
  clone.querySelector('.languages').textContent = `Мови: ${country.languages ? Object.values(country.languages).join(', ') : '—'}`;
